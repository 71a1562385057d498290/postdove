...
Domain and mailbox management can be performed from the **CLI** through the **postnimda** management tools located in the **postnimda** directory.  
The management tools assume **one entry per line**. If you intend to have multiline entries, such as alias definitions stretched on multiple lines, you should **not** use the management tools.

No SQL backend is being used. All configuration is managed using plain text files.  

**Note that no webmail and no web accessible management interfaces are installed !**
